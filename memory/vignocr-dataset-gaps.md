...
**v2c Claude API variant (added 2026-06-23):** a 4th comparison variant `claude` (configs/v2/claude.yaml, src/vignocr/v2/claude_extract.py) — Claude vision reads the vignette image→JSON via the anthropic SDK (default claude-opus-4-8, structured outputs, adaptive thinking low-effort, system-prompt cached), zero training. `pipeline.variant=claude` / VIGNOCR_PIPELINE_VARIANT=claude; `[claude]` extra = anthropic SDK. CONSTRAINT: needs network + ANTHROPIC_API_KEY, so it CANNOT run on Narval's offline compute nodes — run it on the LOGIN node (or cloud backend in prod): `pip install -e .[claude]; export ANTHROPIC_API_KEY=...; python -m vignocr.v2.compare --variants claude --dataset $VLM_DATASET --split test --merge <on-cluster compare_report.json> --out <dir>` (compare.py `--merge` folds it into the on-cluster v1/v2a/v2b report for one table). PRIVACY: sends vignette images to the API — clear data governance before real pharmacy data (API doesn't train by default; ZDR available). It's also the natural PRODUCTION identity path given the SaaS target.
