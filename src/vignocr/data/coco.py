...
def load_split(root: Path | str, split: str) -> CocoSplit:
    ...
    # Real-data reconciliation: data.yaml may alias COCO category names to schema
    ...
    if aliases or ignore:
        ...
        categories = [
            {**c, "name": aliases.get(c["name"], c["name"])}
            for c in categories
    ...
    annotations = [
        a for a in data.get("annotations", []) if int(a["category_id"]) in cat_id_to_name
    ]
