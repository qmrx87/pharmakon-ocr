...
def run(cfg_path: str, run_dir: Path) -> Path:
    ...
    if backend == "baseline":
        ...
        raise ValueError(f"unknown OCR backend {backend!r}; expected 'baseline' or 'transformer'")

    _write_manifest(
        run_dir, cfg, dataset, backend, ckpt, seed, len(train_examples), len(val_examples)
    )
...
def _collect_crops(dataset: dict[str, Any], split: str) -> list[dict[str, Any]]:
    ...
    Each example is ``{"name", "type", "orientation", "image", "bbox", "label"}``.
    The *text label* is the gold transcription, read from the fixture's
    ``ground_truth.json`` (keyed by image ``file_name`` -> ``fields``) and joined
    by field name; real datasets without that sidecar yield ``label=None``.
    """
    # Lazy import: the data subpackage owns COCO loading + cropping.
    from vignocr.data import coco, load_ground_truth  # noqa: PLC0415
    ...
    try:
        gold_by_image = load_ground_truth(root)
    except FileNotFoundError:
        gold_by_image = {}
    # Resolve categories by the COCO file's OWN id->name map (robust to Roboflow ids).
    ...
    for image in split_data.images:
        ...
        # crops_for_image -> {field_name: [Crop(name, bbox, image), ...]}
...
def _train_paddle(
    ...
    for split_name, examples in (("train", train_examples), ("val", val_examples)):
        ...
        for i, ex in enumerate(examples):
            prepared = preprocess_for_type(
                ex["image"], ex["orientation"], field_types.get(ex["type"], {})
            )
    ...
    rec_cfg = {
        ...
        "Train": {
            "dataset": {
                ...
                "label_file_list": [str(data_root / "train_label.txt")],
            },
        ...
        "Eval": {
            "dataset": {
                "data_dir": str(data_root),
                "label_file_list": [str(data_root / "val_label.txt")],
            },
        },
    }
...
def _train_transformer(
    ...
    # Standard VisionEncoderDecoder fine-tuning setup: tie the decoder's special
    # tokens to the processor's tokenizer so generation/loss use the right ids.
    tokenizer = getattr(processor, "tokenizer", processor)
    model.config.decoder_start_token_id = tokenizer.cls_token_id
    model.config.pad_token_id = tokenizer.pad_token_id
    model.config.max_length = int(variant.get("max_new_tokens", 32))
    ...
    # TODO(transformer-train-api): build the torch Dataset from ``train_examples``
    # (each carries ``image`` + gold ``label``) and instantiate the trainer:
    #     trainer = Seq2SeqTrainer(model=model, args=args, train_dataset=..., ...)
    #     trainer.train()
    # The collation differs between TrOCR (pixel_values + tokenizer labels) and
    # Donut (task-prompt-conditioned target sequences). Confirmed at the TrOCR
    # level; the Donut target-sequence construction is assumed and unverified.
    # The dataset/collator wiring is deferred to the GPU training run (Phase 7).
    assert Seq2SeqTrainer is not None  # imported here; instantiated once datasets exist
    log.info(
