    ...
    *,
    ...
    for ex in examples:
        ...
        if gold is None:
            ...
            continue
        read = recognizer.read(ex["image"], ex["type"], ex["orientation"], flow=flow, field_name=ex["name"])
        pred = read.value or ""
        abstained = read.status == "abstain"

        for acc in (overall, agg_type.setdefault(ex["type"], _Acc()), agg_field.setdefault(ex["name"], _Acc())):
...
def _collect_labeled_crops(dataset: dict[str, Any], split: str, schema: Any) -> list[dict[str, Any]]:
    """Crop every annotated text field in ``split``, attaching gold labels.

    Uses the ``data`` subpackage's documented COCO API (docs/INTERFACES.md),
    lazy-imported so this entrypoint imports without the data deps resolved.
    """
